...
int Watchdog::enable(int maxPeriod, bool forSleep)
{
    ...
    if((maxPeriod >= 16000) || !maxPeriod) {
        cycles = 16384;
        bits   = 0xB;
    } else {
        cycles = (maxPeriod * 1024L + 500) / 1000; // ms -> WDT cycles
        if(cycles >= 8192) {
            cycles = 8192;
            bits   = 0xA;
        } else if(cycles >= 4096) {
    ...
    result = (cycles * 1000L + 512) / 1024; // WDT cycles -> ms
...
{
    uint32_t result = enable(period, true); // true = for sleep
